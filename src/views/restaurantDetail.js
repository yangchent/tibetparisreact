...
const getUsers = async (id) => {
	...
	try {
		const res = await axios.get(`http://127.0.0.1:8000/restaurant/${id}/`, 				
		)
...
return (
	<div className="relative container mx-auto md:mt-16">     
		...
		<div key={restaurant.id} className="relative">
			...
			<div className="flex flex-col">
				...
				<div className="relative place-items-center w-full md:w-1/2 md:flex ">
					<img src={restaurant.image} alt="resto" className="border-2 rounded-lg sm:w-2/3 m-1 p-2" />
					<img src={restaurant.image} alt="resto" className="border-2 rounded-lg sm:w-2/3 m-1 p-2" />
					<img src={restaurant.image} alt="resto" className="border-2 rounded-lg sm:w-2/3 m-1 p-2" />
				</div>
				{/* 2nd flex */}
				<div className="w-full sm:flex sm:justify-between whitespace-nowrap space-y-2 p-3 mb-12 bg-gray-50 shadow-lg rounded-xl">
					{/* sub flex 1 */}
					<div className="px-6 font-poppins p-6">
						<h2 className="text-center text-xl text-myorange divide-y pb-2">Address </h2>
						<p className=" text-gray-500 text-2xl pb-2">{restaurant.address}</p>
						<p className="text-2xl text-gray-600">{restaurant.city} {restaurant.Zip_code}</p>
					</div>
					...
					<div className="p-2">								
						{restaurant.link ? (
							<a href={restaurant.link} target="_blank" rel="noreferrer" className="text-blue-600 text-xl md:block hover:text-blue-400">
								<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" className="w-8 h-8 inline-block mr-1" viewBox="0 0 52 52" enable-background="new 0 0 52 52">
								...
								</svg>{restaurant.name}</a>
							) : null 
							}
					</div>
					{/* sub flex 2 b*/}
					<div className="font-poppins flex flex-col pr-12">
						<div className="flex justify-center items-center p-2">
							{restaurant.deliveroo ? (
								<a href={restaurant.deliveroo} target="_blank"  rel="noreferrer" className="text-blue-600 text-2xl hidden md:block hover:text-blue-400">
									<svg xmlns="http://www.w3.org/2000/svg" className='w-24' viewBox="-22.8 232.4 490.8 131.6" >
						...
						<div className=" flex justify-center items-center p-2">
							{restaurant.uber ? (
								<a href={restaurant.uber} target="_blank" rel="noreferrer" className="text-blue-600 text-xl hidden md:block hover:text-blue-400">
									<svg xmlns="http://www.w3.org/2000/svg" className='w-24' viewBox="-.5 .2 1052.9 169.6" >
